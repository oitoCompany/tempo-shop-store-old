...
export default function App() {
  ...
  const handleSearch = (event) => {
    ...
    if (response.data) {
            ...
            let index = cart.findIndex(x => x.matnr == response.data["CATALOG"][0].MATNR);
            //  ? cart.findIndex(x => x.matnr == response[0].MATNR) : -1;
            if (index !== -1) {
              let count = Eligibility.filter(a => a.CATEGORY === cart[index].eligibility)[0].QTY;
              if(count >= response.data["CATALOG"][0].KPEIN){ 
               cart[index].quantity++;  
               setcart(cart);
               sumTotalAmount(cart);
               handelShowCart();
               Eligibilityinde('add', cart[index]);
               console.log("search add cart ", cart , response)
           }
          }
